...
\end{split}
\end{equation*}
The distribution $g(T^*_j)$ depends on the observed longitudinal history $\mathcal{Y}_j(s)$ of patient $j$ via the random effects $\bmath{b}_j$, and on the information from the original dataset $\mathcal{D}_n$ via the posterior distribution of the parameters $p(\bmath{\theta} \mid \mathcal{D}_n)$, where $\bmath{\theta}$ denotes the vector of all parameters.