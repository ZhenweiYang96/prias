...
% In case no GR is observed at $u$, the process is to be repeated a total $N_j^b$ times until GR is detected at a future time $T^b_{j{N_j^b}}$. 
%$T^b_{j{N_j^b}}$ is the time at which GR is detected and $N_j^b$ is the number of biopsies conducted until that time. In this context, the most useful biopsy schedule for a patient will be the one with the least number of biopsies $N_j^b$ and the smallest offset $O_j = T^b_{j{N_j^b}} - T_j^*$ possible. 
...
\input{mainmatter/pers_sched_framework/algorithm}
