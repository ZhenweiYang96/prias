...
\node (takePSA) [process, below=1cm of start] {
...
\node (propTime) [process, below=1cm of takePSA] {
\begin{enumerate}[noitemsep,topsep=0pt, leftmargin=*]
\item Set $u^{pv} = u$ 
\item Update $g(T^*_j)$ 
\item Propose $u$
...
\node (pro3) [process, below=1cm of decision2] {
\begin{enumerate}[noitemsep,topsep=0pt, leftmargin=*]
...
\node (pro4) [process, below=1cm of decision4] {
\begin{enumerate}[noitemsep,topsep=0pt, leftmargin=*]
...
\node (decision3) [decision, below=1cm of pro3] {Gleason > 6?};
...
\draw [arrow] (decision4.east) |- ([xshift=0.35cm, yshift=-4cm]pro6.north east) |- node[anchor=south] {Yes} (takePSA);
...
\draw [arrow] (pro7.west)|- ([xshift=-0.35cm, yshift=-7cm]pro5.north west) |- (propTime);
\end{tikzpicture}
