...
inv_time_to_biopsy_scales = unique(c(10*0.5, 7*0.7, 5*1.1, 4*1.8, 4*0.7, 3*0.8))
names(inv_time_to_biopsy_scales) = c("Annual", "1pt5years", "Biennial", "Triennial",
                                "PRIAS_risk10pc", "risk15pc")

ct = makeCluster(max_cores)
registerDoParallel(ct)
sim_res = vector("list", length(inv_time_to_biopsy_scales))
names(sim_res) = names(inv_time_to_biopsy_scales)
for(i in 1:length(inv_time_to_biopsy_scales)){
  ...
  for(decision_epoch in decision_epochs){
    ...
    for(max_decision_epoch in max_decision_epochs[[decision_epoch_str]]){
      ...
      sim_res[[i]][[decision_epoch_str]][[max_decision_epoch_str]] = 
        foreach(j=1:length(dataset), 
                ...
                .packages = c("splines", "JMbayes")) %dopar%{
                  ...
                  inv_time_to_biopsy_scale = inv_time_to_biopsy_scales[i]
      ...
      save(sim_res, file = paste0("Rdata/mdp/reward_by_time/sim_res_", 
                                  names(inv_time_to_biopsy_scales)[i], ".Rdata"))
    }
  }
}
stopCluster(ct)
