...
reward_matrix = as.matrix(expand.grid(c(6/12, 12/12, 18/12, 24/12),
                                      ...
                                      -1))
reward_matrix = matrix(reward_matrix[9,], ncol = 4)
    ...
    for(j in 1:length(discount_factors)){
      ...
      for(q in 1:length(max_biopsies_vec)){
        ...
        for(max_depth in max_depths){
          ...
          pidl = 15
          sim_res[[i]][[j]][[q]][[max_depth + 1]] = testData$testDs.id[pidl, c("P_ID", "Age", "progression_time")]
          ...
          #save(sim_res, file = paste0("Rdata/mdp/schedule_by_time_y/sim_res_",file_num, ".Rdata"))
        }
      }
    }
  }
