prederrJM_mod = function (object, newdata, Tstart, Thoriz, lossFun = c("square", 
                                                       "absolute"), interval = FALSE, idVar = "id", M = 100, ...) 
{
  ...
  is_counting <- attr(SurvT, "type") == "interval"
  Time <- if (is_counting) {
  ...
  deadThoriz <- newdata2[Time <= Thoriz & (delta == 1 | delta == 
                                             3) & timesInd, ]
  ...
  prederr <- if (length(unique(Time)) > 1 && nrow(aliveThoriz) > 
                 1 && nrow(deadThoriz) > 1) {
    Surv.aliveThoriz <- if (is_counting) {
      survfitJM(object, newdata = aliveThoriz, idVar = idVar, 
                M = M, survTimes = Thoriz, last.time = rep(Tstart, 
                                                           length(idalive)), LeftTrunc_var = all.vars(TermsT)[1L])
    }
    else {
      survfitJM(object, newdata = aliveThoriz, idVar = idVar, 
                M = M, survTimes = Thoriz, last.time = rep(Tstart, 
    ...
    Surv.deadThoriz <- if (is_counting) {
      survfitJM(object, newdata = deadThoriz, idVar = idVar, 
                survTimes = Thoriz, last.time = rep(Tstart, 
    ...
    Surv.aliveThoriz <- sapply(Surv.aliveThoriz$summaries, 
                               "[", 2)
    Surv.deadThoriz <- sapply(Surv.deadThoriz$summaries, 
                              "[", 2)
    if (nrow(censThoriz)) {
      Surv.censThoriz <- if (is_counting) {
        survfitJM(object, newdata = censThoriz, idVar = idVar, 
                  M = M, survTimes = Thoriz, last.time = rep(Tstart, 
                                                             length(idcens)), LeftTrunc_var = all.vars(TermsT)[1L])
      }
      else {
        survfitJM(object, newdata = censThoriz, idVar = idVar, 
                  M = M, survTimes = Thoriz, last.time = rep(Tstart, 
      ...
      weights <- if (is_counting) {
        survfitJM(object, newdata = censThoriz, idVar = idVar, 
                  M = M, survTimes = Thoriz, last.time = tt[!duplicated(censThoriz[[idVar]])], 
    ...
    if (!interval) {
      (1/nr) * sum(lossFun(1 - Surv.aliveThoriz), lossFun(0 - 
                                                            Surv.deadThoriz), weights * lossFun(1 - Surv.censThoriz) + 
      ...
      times <- TimeCens[TimeCens > Tstart & TimeCens < 
                          Thoriz & !deltaCens]
      times <- sort(unique(times))
      k <- as.numeric(table(times))
      w <- summary(KMcens, times = Tstart)$surv/summary(KMcens, 
                                                        times = times)$surv
      prederr.times <- sapply(times, function(t) prederrJM(object, 
                                                           newdata, Tstart, t, M = M, interval = FALSE, 
                                                           idVar = idVar)$prederr)
  ...
  out <- list(prederr = prederr, nr = nr, Tstart = Tstart, 
              Thoriz = Thoriz, interval = interval, classObject = class(object), 
              nameObject = deparse(substitute(object)), lossFun = lf)
  class(out) <- "prederrJM"
  out
}

environment(prederrJM_mod) <- asNamespace('JMbayes')