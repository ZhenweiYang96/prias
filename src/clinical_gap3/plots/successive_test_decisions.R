...
conditionalDynamicRiskPlot = function(object, pat_df, latest_survival_time=NA, 
  ...
  p = ggplot() +  
    ...
    scale_color_manual(values = c(THEME_COLOR), labels="Observed PSA (log scale)")
...
label_plot = ggplot() + 
  ...
  geom_label(aes(x=c(2.5, 4.5,6.5), y=c(-0.07,0,0), 
                 label = c("No biopsy","No biopsy", "No biopsy")),
             size= LABEL_SIZE) +
...
print(schedule_plot)
ggsave(schedule_plot, filename = "report/clinical/images/successive_biopsy_decisions_102.eps",
       device = cairo_ps,  height = 4.5, width=6.75)
