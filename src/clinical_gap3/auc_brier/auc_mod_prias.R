aucJM.mvJMbayes_mod <- function (object, newdata, Tstart, Thoriz = NULL, Dt = NULL, 
                                 idVar = "id", M = 100, ...) {
  ...
  orderedTime = Time[ordTime]
  newdata2 <- newdata2[orderedTime > Tstart, ]
  if(nrow(newdata2)==0){
    return(list(auc=NA))
  }
  #newdata2 <- newdata2[newdata2[[timeVar]] <= Tstart, ]
  newdata2_max_Thoriz <- newdata2[newdata2[[timeVar]] <= Thoriz, ]
  newdata2_max_Tstart <- newdata2[newdata2[[timeVar]] <= Tstart, ]
  pi.u.t <- if (is_counting) {
    survfitJM(object, newdata = newdata2, idVar = idVar, survTimes = Thoriz, M = M, 
              LeftTrunc_var = all.vars(TermsT)[1L])
  } else {
    if(is.factor(newdata2_max_Tstart$P_ID)){
  ...
  id <- newdata2_max_Thoriz[[idVar]]
  SurvT <- model.response(model.frame(TermsT, newdata2_max_Thoriz)) 
  if (is_counting) {
  ...
  auc <- if (length(Time) > 1) {
    ...
    if (any(ind2)) {
      ...
      pi2 <- if (is_counting) {
        ...
        survfitJM(object, newdata = newdata2_max_Thoriz[id %in% unq_nams_i, ], idVar = idVar, 
                  last.time = Time[unq_nams_i], survTimes = Thoriz, M = M, seed=2019)
      }
    ...
    if (any(ind3)) {
      ...
      pi3 <- if (is_counting) {
        ...
        survfitJM(object, newdata = newdata2_max_Thoriz[id %in% unq_nams_j, ], idVar = idVar, 
                  last.time = Time[unq_nams_j], survTimes = Thoriz, M = M, seed=2019)
      }
    ...
    if (any(ind4)) {
      ...
      if (is_counting) {
        ...
        pi4_i <- survfitJM(object, newdata = newdata2_max_Thoriz[id %in% unq_nams_i, ], 
                           idVar = idVar, last.time = Time[unq_nams_i], survTimes = Thoriz, 
                           M = M, seed=2019)
        pi4_j <- survfitJM(object, newdata = newdata2_max_Thoriz[id %in% unq_nams_j, ], 
                           idVar = idVar, last.time = Time[unq_nams_j], survTimes = Thoriz, 
                           M = M, seed=2019)
      }
...
environment(aucJM.mvJMbayes_mod) <- asNamespace('JMbayes')