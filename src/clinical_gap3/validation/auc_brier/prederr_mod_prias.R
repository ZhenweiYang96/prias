prederrJM.mvJMbayes_mod <- function (object, newdata, Tstart, Thoriz, lossFun = c("square", "absolute"), 
  ...
  timesInd_max_Tstart <- newdata2[[timeVar]] <= Tstart
  timesInd_max_Thoriz <- newdata2[[timeVar]] <= Thoriz
  
  aliveThoriz <- newdata2[Time > Thoriz & timesInd_max_Thoriz, ]
  deadThoriz <- newdata2[Time <= Thoriz & (event == 1 | event == 3) & timesInd_max_Thoriz, ]
  indCens <- Time < Thoriz & (event == 0 | event == 2) & timesInd_max_Thoriz
  ...
  prederr <- if (length(unique(Time)) > 1 && nrow(aliveThoriz) > 1 &&
    ...
    Surv.aliveThoriz <- if (is_counting) {
      ...
      aliveTstart = newdata2[Time > Thoriz & timesInd_max_Tstart, ]
      
      if(is.factor(aliveTstart$P_ID)){
      ...
      survfitJM(object, newdata = aliveThoriz, idVar = idVar, M = M,
                survTimes = Thoriz, last.time = lastTime, seed=2019)
    ...
    Surv.deadThoriz <- if (is_counting) {
      ...
      deadTstart =  newdata2[Time <= Thoriz & (event == 1 | event == 3) & timesInd_max_Tstart, ]
      if(is.factor(deadTstart$P_ID)){
    ...
    if (nrow(censThoriz)) {
      Surv.censThoriz <- if (is_counting) {
        ...
        censTstart =  newdata2[Time < Thoriz & (event == 0 | event == 2) & timesInd_max_Tstart, ]
        
        if(is.factor(censTstart$P_ID)){
          censTstart$P_ID = droplevels(censTstart$P_ID)
        }
      ...
      weights <- if (is_counting) {
        ...
        survfitJM(object, newdata = censThoriz, idVar = idVar, M = M,
                  survTimes = Thoriz, last.time = tt[!duplicated(censThoriz[[idVar]])], seed=2019)
      }
