                  ...
                  sidebarLayout(
                    ...
                    mainPanel(
                      tabsetPanel(type = "tabs",
                                  ...
                                  tabPanel("Cumulative-risk of Gleason ≥ 7", tags$br(),
                                           sliderInput("risk_pred_time", "Follow-up time since current visit:",
                                                       min = as.POSIXct(0, origin=SPSS_ORIGIN_DATE), 
                                                       max = as.POSIXct(MAX_FOLLOW_UP*YEAR_DIVIDER, origin=SPSS_ORIGIN_DATE), 
                                                       value = as.POSIXct(0, origin=SPSS_ORIGIN_DATE), step = YEAR_DIVIDER*0.5, timeFormat="%b %Y",
                                  ...
                                  tabPanel("Biopsy recommendation", 
                                           ...
                                           tags$br(),
                                           tags$span('More information on the definition of the delay can be found in Section 6 of "Documentation" tab.', class='label label-default'),
                                           plotOutput("biopsy_delay_gauge_graph")),
                                  tabPanel("Documentation", 
                                           tags$h3(tags$span("The risk predictions, biopsy schedules, and expected delays in detection of Gleason ≥ 7 are only indicative.", class='label label-warning')),
                                           tags$h3(tags$span("Clinical correlation is recommended.", class='label label-warning')),
                                           tags$hr(),
                                           tags$h2(tags$span("1. Calculation of cumulative-risk of Gleason ≥ 7", class='label label-success')),
                                           tags$br(),
                                           ...
                                           tags$span("More specifically, they schedule a biopsy at a follow-up visit if the cumulative-risk of Gleason ≥ 7 at a visit is higher than a certain threshold (e.g., 10% risk).", class='lead'),
                                           tags$span("Biopsies are scheduled sequentially, and after each decision of biopsy the cumulative-risk profile is updated to account for the possibility of not finding Gleason ≥ 7 during the biopsy.", class='lead'),
                                           tags$hr(),
                                           tags$h2(tags$span("5. Biopsies are scheduled only for the first 10 years of follow-up", class='label label-success')),
                                           tags$span("We schedule biopsies only for the first ten years follow-up because of limited follow-up period of PRIAS, which is used as the training dataset.", class='lead'), 
                                           tags$span("A compulsory biopsy was scheduled at year ten of follow-up in all schedules for meaningful comparison of their expected delays in detection of Gleason ≥ 7.", class='lead'),
                                           tags$hr(),
                                           tags$h2(tags$span("6. Expected time delay in detection of Gleason ≥ 7", class='label label-success')),
                                           tags$span("In active surveillance biopsies are conducted intermittently. Hence, Gleason ≥ 7 is always detected with a time delay as shown in the Figure below.", class='lead'),
                                           ...
                                           tags$span("Since Gleason ≥ 7 occured at year 3.25 (Apr 2008), it could only be detected after a delay of 15 months at year 4.5 (Jul 2009).", class='lead'),
                                           ...
                                           tags$span("However, the expected time delay in detection of Gleason ≥ 7 is only valid under the condition that the patient obtains Gleason ≥ 7 within the first 10 years of follow-up.", class='lead'),
                                           tags$hr(),
