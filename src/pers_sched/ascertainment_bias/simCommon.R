...
getProgressionStartStopTime = function(progressionTime, patientDs){
  fixedSchedule = c(1, 4, 7, 10, 15, 20, 25, 30, 35, 40, 45, 50)
  
  offset = -Inf
  
  lastBiopsyTime = 0.00001
  proposedBiopsyTime = Inf
  
  if(progressionTime < 1){
    return(c(progression_time_start = lastBiopsyTime, progression_time_end = 1))
  }
  ...
  for(j in 4:nrow(patientDs)){
    ...
    if(curVisitTime > proposedBiopsyTime){
      ...
      if(offset > 0){
        return(c(progression_time_start = lastBiopsyTime, progression_time_end = proposedBiopsyTime))
      }
...
generateSimJointData = function(dsId, nSub, psa_dt_yes = F, intervalCensoring = T){
  u <- runif(nSub)
  ...
  if(intervalCensoring==T){
    testDs$progression_time = rep(testDs.id$progression_time, each=timesPerSubject)
    
    #Fixed schedule interval censoring times
    if(psa_dt_yes){
      trainingDs.id[,c("progression_time_start", "progression_time_end")] = t(sapply(1:nrow(trainingDs.id), 
                                                                                     function(patientRowNum){
                                                                                       progressionTime = trainingDs.id$progression_time[patientRowNum]
                                                                                       patientId = trainingDs.id$P_ID[patientRowNum]
                                                                                       patientDs = trainingDs[trainingDs$P_ID == patientId,]
                                                                                       
                                                                                       getProgressionStartStopTime(progressionTime, patientDs)
                                                                                     }))
    }
    else{
      fixedBiopsyTimes = c(0.00001,1,4,7,10,15,20,25)
      trainingDs.id$progression_time_start = sapply(trainingDs.id$progression_time, function(target){
        tail(fixedBiopsyTimes[which(fixedBiopsyTimes<=target)],1)
      })
      trainingDs.id$progression_time_end = sapply(trainingDs.id$progression_time, function(target){
        fixedBiopsyTimes[which(fixedBiopsyTimes>=target)][1]
      })
    }
    
    trainingDs$progression_time_end = rep(trainingDs.id$progression_time_end, each=timesPerSubject)
    
    # drop the longitudinal measurementsthat were taken after the observed event time for each subject.
    trainingDs = trainingDs[trainingDs$visitTimeYears <= trainingDs$progression_time_end, ]
...
fitJointModelSimDs = function(trainingDs.id, trainingDs, intervalCensoring=T){
  
  if(intervalCensoring == T){
    surv_Model_training = survreg(Surv(progression_time_start, progression_time_end, type = "interval2") ~ 
                                    I(Age - 70) +  I((Age - 70)^2), data = trainingDs.id, model = TRUE)
    
  }else{
    surv_Model_training = coxph(Surv(progression_time, progressed) ~ I(Age - 70) +  I((Age - 70)^2),
  ...
  mvJoint_psa_tdboth_training <- mvJointModelBayes(mvglmer_psa_training, surv_Model_training, timeVar = "visitTimeYears",
  ...
  jmbayes_psa_tdval_training = jmbayes_psa_tdboth_training = simJointModel_replaced = NULL
  
  
  list(mvJoint_psa_tdval_training = mvJoint_psa_tdval_training, 
       mvJoint_psa_tdboth_training = mvJoint_psa_tdboth_training,
       surv_Model_training = surv_Model_training,
...
alphas = c(getAlpha(fittedJointModel, weightedOnes = F))