...
for(i in 1:totalSizeBootstrap){
  while(T){
    ...
    psa_data_set_i$high_dre = ifelse(psa_data_set_i$dre=="T1c", 0, 1)
  ...
  forms_dre_value = list("log2psaplus1" = "value",
  ...
  auc_no_dre[[i]] = vector("list", 3)
  times = 1:9 + 0.0001
  deltaT = 1.0101
  
  j=1
  for(Tstart in times){
    auc_no_dre[[i]][[j]] = aucJM(new.mvJoint_no_dre, psa_data_set_i,
  ...
  aucBootStrapped = list("no_dre" = auc_no_dre)
