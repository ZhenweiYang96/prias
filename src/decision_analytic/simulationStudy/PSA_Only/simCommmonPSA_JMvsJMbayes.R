...
getBaselineHazard = function(progression_speed, times){
  weibullScale = switch(progression_speed, "Fast"=3, "Medium"=5, "Slow"=8)
  weibullShape = switch(progression_speed, "Fast"=5, "Medium"=8, "Slow"=14)
  #weibullScale = switch(progression_speed, "Fast"=3, "Medium"=3, "Slow"=3)
  #weibullShape = switch(progression_speed, "Fast"=5, "Medium"=5, "Slow"=5)
  
  return((weibullShape/weibullScale)*(times/weibullScale)^(weibullShape-1))
}

mvJoint_psa_light$mcmc$b = NULL
gammas = mvJoint_psa_light$statistics$postwMeans$gammas
alphas = mvJoint_psa_light$statistics$postwMeans$alphas
betas_psa = mvJoint_psa_light$statistics$postwMeans$betas1
sigma_psa = mvJoint_psa_light$statistics$postwMeans$sigma1
D = mvJoint_psa_light$statistics$postwMeans$D
...
fitJointModelOnNewData = function(seed, nSub, nSubTraining, nSubTest, mvglmer_iter=1000, 
  ...
  hazardFunc = function (s, patientId) {
    ...
    baselinehazard_s = getBaselineHazard(progression_speed, s)
    ...
    zib_log2psaplus1_val = zi_s_log2psaplus1_val %*% b_subject
    zib_log2psaplus1_slope = zi_s_log2psaplus1_slope %*% b_subject[-1] #One less random effect to ignore intercept
    ...
    y_Alpha = cbind(xBetaZb_s_log2psaplus1_value, xBetaZb_s_log2psaplus1_slope) %*% alphas
  ...
  simDs.id$progression_time = foreach(i=1:nSub,.combine='c',
                                      .export=c("pSurvTime", "invSurvival", "hazardFunc", "MAX_FAIL_TIME","getBaselineHazard",
                                                ...
                                                "randomPSASlopeFormula", "betas_psa", "alphas", "b", "simDs.id"),
  ...
  mvglmer_psa_simDs = mvglmer(list(log2psaplus1 ~ I(Age - 70) +  I((Age - 70)^2) +
                                         ...
                                         (ns(visitTimeYears, knots=c(0.1, 0.7), Boundary.knots=c(0, 5.42))|P_ID)),
                                  data=trainingDs, families = list(gaussian), engine = "JAGS",
  ...
  forms_simDs = list("log2psaplus1" = "value",
                     "log2psaplus1" = list(fixed = ~ 0 + dns(visitTimeYears, knots=c(0.1, 0.7, 4), Boundary.knots=c(0, 5.42)),
                                           random=~0 + dns(visitTimeYears, knots=c(0.1, 0.7), Boundary.knots=c(0, 5.42)),
                                           indFixed = 4:7, indRandom=2:4, name = "slope"))
  ...
  lmeFit <- lme(log2psaplus1 ~ I(Age - 70) +  I((Age - 70)^2) + ns(visitTimeYears, knots=c(0.1, 0.7, 4), Boundary.knots=c(0, 5.42)),
                random = ~ns(visitTimeYears, knots=c(0.1, 0.7), Boundary.knots=c(0, 5.42))|P_ID, 
                data = trainingDs,
...
# 
# fit_hazard = splineDesign(mvJoint_dre_psa_simDs$control$knots, times, 
#                           ord = mvJoint_dre_psa_simDs$control$ordSpline, outer.ok = T) %*% mvJoint_dre_psa_simDs$statistics$postMeans$Bs_gammas
