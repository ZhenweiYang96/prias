...
survivalFormula = ~ 0 + I(Age - 70) + I((Age - 70)^2) + high_dre_log_odds
...
fitJointModelOnNewData = function(seed, nSub, nSubTraining, nSubTest, 
  ...
  weibullScales = c("Fast"=3, "Medium"=4, "Slow"=5)
  weibullShapes = c("Fast"=5, "Medium"=6.5, "Slow"=8)
  
  # weibullScales = c("Fast"=4,"Medium"=5,"Slow"=6)
  # weibullShapes = c("Fast"=1.5,"Medium"=3,"Slow"=4.5)
  ...
  hazardFunc = function (s, patientId) {
    ...
    #df_s = data.frame(visitTimeYears = rep(1, length(s)), Age = Age)
    ...
    xBetaZb_s_log2psaplus1_slope = xi_s_log2psaplus1_slope %*% betas_psa[4:7] + zib_log2psaplus1_slope #-c(1:3) to ignore intercept, age and age^2
    
    y_Alpha = cbind(xBetaZb_s_logodds_high_dre_value, xBetaZb_s_log2psaplus1_value, xBetaZb_s_log2psaplus1_slope) %*% alphas
    #y_Alpha = cbind(xBetaZb_s_logodds_high_dre_value) %*% alphas[1]
    #y_Alpha = cbind(xBetaZb_s_log2psaplus1_value, xBetaZb_s_log2psaplus1_slope) %*% (alphas[2:3] + c(0, 3))
  ...
  pSurvTime_notimedep = function(survProb, patientId){
    wGamma = simDs.id$wGamma[patientId]
    progression_speed = simDs.id$progression_speed[patientId]
  ...
  simDs.id$visitTimeYears = 1
  simDs.id$high_dre_log_odds = model.matrix(fixedDREFormula, data=simDs.id) %*% betas_dre + rowSums(model.matrix(randomDREFormula, data=simDs.id) * b[, 1:2])
  simDs.id$wGamma <- as.vector(model.matrix(survivalFormula, data = simDs.id) %*% c(gammas, alphas[1]))
  ...
  simDs.id$progression_time = foreach(i=1:nSub,.combine='c', 
                                      ...
                                      .packages = c("splines", "JMbayes")) %dopar%{
                                        pSurvTime_notimedep(simDs.id$survProbs[i], i)
                                      }
  ...
  #Save value and velocity at failure time for each patient
  simDs.id$visitTimeYears = simDs.id$progression_time
  simDs.id$visitTimeYears = 1
  options(na.action="na.pass")
  simDs.id$log2psa_value_fail_time = model.matrix(fixedPSAFormula, data=simDs.id) %*% betas_psa + rowSums(model.matrix(randomPSAFormula, data=simDs.id) * b[, 3:7])
  simDs.id$log2psa_slope_fail_time = model.matrix(fixedPSASlopeFormula, data=simDs.id) %*% betas_psa[4:7] + rowSums(model.matrix(randomPSASlopeFormula, data=simDs.id) * b[, 4:7])
  simDs.id$high_dre_logodds_fail_time = model.matrix(fixedDREFormula, data=simDs.id) %*% betas_dre + rowSums(model.matrix(randomDREFormula, data=simDs.id) * b[, 1:2])
  ...
  survModel_simDs = coxph(Surv(progression_time, progressed) ~ 
                            I(Age - 70) +  I((Age - 70)^2) + 
                            #log2psa_value_fail_time, 
                            #log2psa_slope_fail_time +
                            high_dre_log_odds,
                          data = trainingDs.id, x=T, model=T)
  
  print("Starting to fit joint model with mvJointModelBayes")
  startTime_mvJoint_simDs = Sys.time()
  mvJoint_dre_psa_simDs = NULL
