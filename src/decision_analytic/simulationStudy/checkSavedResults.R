...
for(i in 1:length(savedFiles)){
  ...
  mcmcRes[[i]] = jointModelData$mvJoint_dre_psa_simDs$mcmc
  weights[[i]] = jointModelData$mvJoint_dre_psa_simDs$weights
...
sapply(1:nrow(betas1), function(i){
  boxplot(betas1[i,])
  abline(h = mvJoint_dre_psa_dre_value_light$statistics$postwMeans$betas1[i])
})
...
sapply(1:nrow(alphas), function(i){
  boxplot(alphas[i,])
  abline(h = mvJoint_dre_psa_dre_value_light$statistics$postwMeans$alphas[i])
})
