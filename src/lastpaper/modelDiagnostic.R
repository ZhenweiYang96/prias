...
plotFittedDREMarginal = function(modelObject, logOdds = F, FONT_SIZE=10){
  year_visit = seq(0, 10, 0.5)
  df = data.frame(Age = 70, year_visit)
  
  betas_dre_matrix = modelObject$mcmc$betas1
  D_matrix = modelObject$mcmc$D
  
  fixedDREFormula = ~ 1 + I(Age - 70) +  I((Age - 70)^2) + year_visit
  randomDREFormula = ~ 1 + year_visit
  ...
  plot = ggplot() + 
    geom_line(aes(x=year_visit, y=meanXbetaZb)) +
    geom_ribbon(aes(x=year_visit, ymin = lowerXbetaZb, ymax = upperXbetaZb), fill="grey", alpha=0.5) + 
    theme_bw() + 
...
plotFittedDRESubject = function(modelObject, pid=NA,
  ...
  dreObserved = data$palpable_dre[data$P_ID==pid]
...
temp = list()
for(m in 1:9){
  #temp[[m]] = plotFittedPSASubject(modelObject=mvJoint_dre_psa_3knots_quad_age, FONT_SIZE=13, showTitle=F)
  #temp[[m]] = plotFittedPSASubject(modelObject=mvJoint_dre_psa_2knots_quad_age, FONT_SIZE=13, showTitle=F)
  temp[[m]] = plotFittedPSASubject(modelObject=mvJoint_dre_psa_2knots_lin_age, FONT_SIZE=13, showTitle=F)
}
subjectplot = ggpubr::ggarrange(plotlist = temp, nrow = 3, ncol=3, common.legend = T, legend = "bottom")
...
plotFittedPSASubject = function(modelObject,
                                FONT_SIZE=10, showTitle=T){
  data.id = modelObject$model_info$coxph_components$data
...
plotFittedPSAMarginal = function(modelObject, weighted=F, FONT_SIZE=10){
  ...
  fixedPSAFormula = ~ 1 +I(Age - 70) +  I((Age - 70)^2) + ns(year_visit, knots=c(0.1, 0.7, 4), Boundary.knots=c(0, 5.42))
  ...
  ggplot() + 
    geom_line(aes(x=year_visit, y=meanXbeta)) +
    geom_ribbon(aes(x=year_visit, ymin = lowerXbeta, ymax = upperXbeta), fill="grey", alpha=0.5) + 
    theme_bw() + 
    theme(text = element_text(size=FONT_SIZE), axis.text=element_text(size=FONT_SIZE),
          axis.line = element_line(),
          panel.grid.minor = element_blank()) +  
...
plotPSAResidualVsFitted = function(modelObject){
  data = modelObject$model_info$mvglmer_components$data
  data = data[!is.na(data$log2psaplus1),] 
...
getFittedPSAVelocities = function(fittedJointModel){
  generateTruePSASlope = function(year_visit, randomEff_psa_slope){
    ...
    fixedPSASlopeFormula = ~ 0 + dns(year_visit, knots=c(0.1, 0.7, 4), Boundary.knots=c(0, 5.42))
    randomPSASlopeFormula = ~ 0 + dns(year_visit, knots=c(0.1, 0.7, 4), Boundary.knots=c(0, 5.42))
  ...
  sapply(1:length(unique(data$P_ID)), function(patientRow){
    ...
    generateTruePSASlope(data$year_visit[data$P_ID==uniquePIDs[patientRow]], randomEff_psa_slope)
  })
}
