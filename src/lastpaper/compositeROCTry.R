...
set.seed(2019)
fitted_jm = sim_res$mvJoint_psa_simDs

#First make sure that longitudinal data is available only upto T_horiz
newdata = sim_res$trainingData$trainingDs[sim_res$trainingData$trainingDs$year_visit <= T_horiz,]
newdata$right_cens_time = newdata$progression_time
newdata$reclassification = newdata$progressed==1
...
youden = which.max(results$tpr - results$fpr)
f1 = which.max(2 * results$nTP/(2 * results$nTP + results$nFP + results$nFN))

ggplot() + 
  geom_line(aes(x=results$fpr, y=results$tpr, color='simple')) + 
  geom_point(data=results[results$threshold==0.1,], 
             aes(x=fpr, y=tpr), size=3)+
  geom_label(data=results[youden,], 
             aes(x=fpr, y=tpr, label=round(threshold,3)), size=3)+
  geom_label(data=results[f1,], 
             aes(x=fpr, y=tpr, label=round(threshold,3)), size=3)+
...
thresholds_ = seq(0,1, by=0.001)
threshold_comp = expand.grid(thresholds_[thresholds_<=0.15], 
                             thresholds_[thresholds_<=0.25])
...
ggplot() + geom_line(aes(x=results_comp$fpr, y=results_comp$tpr, color='comp')) + 
  geom_point(data=results[results$threshold==0.1,], 
             aes(x=fpr, y=tpr), size=3)+
  geom_point(data=results[results$threshold==0.15,], 
             aes(x=fpr, y=tpr), size=3)+
  geom_line(aes(x=results$fpr, y=results$tpr, color='simple')) +
  geom_abline(intercept = 0, slope=1) +
  xlim(0,1) + ylim(0,1) + theme(legend.position = "bottom")

tt = results_comp[which(abs(results_comp$fpr - results$fpr[results$threshold==0.1])<= 1e-4 & 
                     results_comp$tpr > results$tpr[results$threshold==0.1]),]
tt[order(tt$tpr, decreasing = T),]
